...
// @ts-ignore
import { exists, removeExt, getExt, renderTemplateToFile } from "./lib"
...
async function mdToHtml(filePath: string) {
  ...
  const replitLinkFile = path.join(dir, 'replitLink.txt');
  let replitLink = "";
  if(existsSync(replitLinkFile)) {
    replitLink = (await readFile(replitLinkFile)).toString()
    if (!replitLink.endsWith('?embed=true')) {
      replitLink += '?embed=true'
    }
  }
  ...
  const markdown = mustache.render(content, replitLink)
